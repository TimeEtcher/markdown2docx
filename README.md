...
- **正文配置**：中文字体、英文字体、字号、行距、首行缩进两字符、段前/段后空行
- **标题配置**：H1–H6 每级独立设置中文字体、字号、加粗、对齐、段前/段后空行、行距；编号与标题之间的间隔可选空格/制表符/两个空格/四个空格
- **字号选择**：GUI 中使用 Word 中文字号（初号、小初、一号、小一、二号、小二、三号、小三、四号、小四、五号、小五、六号、小六、七号、八号），自动映射为磅值
- **样式保存/加载模版**：GUI 内一键保存当前配置为 YAML，下次启动自动加载默认配置，也可手动加载任意配置文件
...
- **保存模版 / 加载模版**：把当前界面设置导出为 YAML，或从 YAML 导入；默认自动加载 `~/.md2word.yaml`
